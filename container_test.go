...
func TestCreateContainerIntegration(t *testing.T) {
	...
	// 7) 같은 이름으로 CreateContainer 를 한 번 더 부르면 에러가 나야 함.
	// CreateContainer 는 create-if-absent 계약이며, 기존 컨테이너를 재사용하는
	// idempotent get-or-create API가 아니다.
	result2, err := CreateContainer(ctx, spec)
	if !errors.Is(err, ErrContainerAlreadyExists) {
		t.Fatalf("expected ErrContainerAlreadyExists on duplicate name, got result=%v err=%v", result2, err)
	}
}
