...
func NewConfig(sourceImageName string) *BuildConfig {
	...
	return &BuildConfig{
		Image: ImageConfig{
			...
			Directories:     []string{ContainerAppDir, ContainerScriptsDir},
			ScriptMap: map[string][]string{
				ContainerAppDir:     {"./executor.sh", "./healthcheck.sh", "./install.sh"},
				ContainerScriptsDir: {"./scripts/user_script.sh"},
			},
			PermissionFiles: []string{
				ContainerExecutorPath,
				ContainerInstallPath,
				ContainerHealthcheckPath,
				ContainerUserScriptPath,
			},
			WorkDir: ContainerAppDir,
			CMD:     DefaultExecutorCommand(),
		},
...
func (config *BuildConfig) CreateImageWithDockerfile(ctx context.Context, store storage.Store) (*buildah.Builder, string, error) {
	...
	builder, err := createConfiguredBuilder(ctx, store, id, config.Image)
	if err != nil {
		return builder, "", err
	}
...
func (c *ContainerConfig) SetupContainer(builder *buildah.Builder) error {
	return applyContainerConfigToBuilder(builder, *c)
}
