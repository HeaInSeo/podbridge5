...
다음으로 `saveImage`와 export 경로를 별도 runtime helper 레이어로 옮깁니다.
이 단계의 목적은 아카이브 경로 계산, writer 준비, 실제 export 호출을 분리해서,
`saveImage`가 orchestration만 담당하도록 만드는 것입니다.

대상:

- `image.go`
- 신규 export runtime helper
- 신규 unit test

완료 기준:

- export 호출이 runtime helper를 통해 수행됨
- 아카이브 파일 경로 계산과 writer 준비가 helper 함수로 분리됨
- `saveImage`는 runtime helper를 호출하는 wrapper로 정리됨
- helper orchestration 로직이 unit test로 검증됨
...
다음으로 `buildconfig.go`에 반복되던 builder 설정 적용 경로를 별도 helper로 옮깁니다.
이 단계의 목적은 `ImageConfig`와 `ContainerConfig`를 builder에 적용하는 조립 흐름과,
commit/save orchestration을 공통 helper로 모아서 호출부 중복을 줄이는 것입니다.

대상:

- `buildconfig.go`
- 신규 buildconfig runtime helper
- 신규 unit test

완료 기준:

- image/container config를 builder에 적용하는 helper가 별도로 존재함
- `CreateImage`, `CreateImageWithDockerfile`, `SetupContainer`가 helper를 사용함
- builder commit/save orchestration이 helper로 분리됨
- helper 조립 로직이 unit test로 검증됨
...
이 문서를 기준으로 이번 턴까지 진행한 구현은 아래 아홉 가지입니다.
...
### 8. Separate image export runtime helpers

Next, move `saveImage` and the export path behind a dedicated runtime helper layer.
The purpose of this step is to separate archive path calculation, writer preparation, and the actual export call,
so `saveImage` only handles orchestration.

Targets:

- `image.go`
- new export runtime helper
- new unit test

Definition of done:

- export calls are performed through a runtime helper
- archive path calculation and writer preparation are moved into helper functions
- `saveImage` is reduced to a wrapper that calls the runtime helper
- the helper orchestration logic is covered by unit tests
...
### 9. Separate BuildConfig assembly helpers

Next, move the repeated builder-application path in `buildconfig.go` into dedicated helpers.
The purpose of this step is to collect the assembly flow that applies `ImageConfig` and `ContainerConfig` to builders,
and the commit/save orchestration, into common helpers so the call sites are less repetitive.

Targets:

- `buildconfig.go`
- new buildconfig runtime helper
- new unit test

Definition of done:

- dedicated helpers exist for applying image/container config to builders
- `CreateImage`, `CreateImageWithDockerfile`, and `SetupContainer` use those helpers
- builder commit/save orchestration is moved into a helper
- the helper assembly logic is covered by unit tests
...
The implementation started from this document up to this turn covered these nine slices:
